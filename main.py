...
BRAVO_GAMEOVER = pygame.transform.scale(pygame.image.load(os.path.join('Assets','BRAVO_GAME_OVER.png')),(20*hw, 4*hh))

ALPHA_GAMEOVER = pygame.transform.scale(pygame.image.load(os.path.join('Assets','ALPHA_GAME_OVER.png')),(20*hw, 4*hh))

FENCE = pygame.transform.scale(pygame.transform.rotate(pygame.image.load(os.path.join('Assets','Fence.png')),90),(200,HEIGHT-380))

EXPLOSSION = pygame.image.load(os.path.join('Assets','EXPLOSSION.gif'))

BULLET_HIT_SOUND = pygame.mixer.Sound('Assets/Grenade+1.mp3')
BULLET_FIRE_SOUND = pygame.mixer.Sound('Assets/Gun+Silencer.mp3')
GAME_OVER_SOUND = pygame.mixer.Sound('Assets/GAME_OVER.mp3')
INTRO_SOUND = pygame.mixer.Sound('Assets/INTRO_SOUND.mp3')


def draw_window(blue, red, blue_bullets, red_bullets, blue_health, red_health, t1, t2):
    WIN.blit(GROUND, (0, 0))
    # pygame.draw.rect(WIN, CUSTOM, BORDER)
    # WIN.blit(FENCE,((WIDTH/2) -90, 420))
    # WIN.blit(EXPLOSSION,((WIDTH/2) -90, 420))
...
def red_handle_movement(keys_pressed, red, t1):
    ...
    if keys_pressed[pygame.K_d] and red.x + v + red.width < BORDER.x-60:  
        red.x += v
    if keys_pressed[pygame.K_w] and red.y - v > 300: 
        red.y -= v
...
def blue_handle_movement(keys_pressed, blue, t2,):
    ...
    if t2 > 0:
        v = (VEL/2)
    if keys_pressed[pygame.K_LEFT] and blue.x - v > BORDER.x + BORDER.width+60: 
        blue.x -= v
    if keys_pressed[pygame.K_RIGHT] and blue.x + v + blue.width < WIDTH:  
        blue.x += v
    if keys_pressed[pygame.K_UP] and blue.y - v > 300: 
        blue.y -= v
...
def draw_winner(winner):
    ...
    if winner == 1:
        ...
        WIN.blit(BRAVO_GAMEOVER, ((WIDTH/2) - 10*hw, (HEIGHT/2) - 5*hw))
        GAME_OVER_SOUND.play()
    else :
        WIN.blit(BRAVO_WIN, ((WIDTH/2) - 10*hw, (HEIGHT/2) - hw))
        WIN.blit(ALPHA_GAMEOVER, ((WIDTH/2) - 10*hw, (HEIGHT/2) - 5*hw))
        GAME_OVER_SOUND.play()
    pygame.display.update()
